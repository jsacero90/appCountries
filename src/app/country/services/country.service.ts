...
export class CountryService
{
  ...
  {
    return this.http.get<CountryResponse[]>( `${ this.apiUrl }/name/${ country }` ).pipe( catchError( _err => of( [] ) ) );
  }

  searchCapitals ( capital: string ): Observable<CountryResponse[]>
  {
    return this.http.get<CountryResponse[]>( `${ this.apiUrl }/capital/${ capital }` ).pipe( catchError( _err => of( [] ) ) );
  }

  searchAlphaCode ( alpha: string ): Observable<any>
  {
    return this.http.get<CountryResponse[]>( `${ this.apiUrl }/alpha/${ alpha }` )
      .pipe( map( ( resp: CountryResponse[] ) =>
      {
